...
def _run_pipeline(transcript: str) -> None:
    ...
    if not result.requirements.has_buildable_scope:
        # Architect/Developer/Code Review/Testing never ran — clear the
        # tracker instead of leaving those steps stuck on "pending".
        tracker_box.empty()
...
if "result" in st.session_state:
    ...
    if not result.requirements.has_buildable_scope:
        ...
        with st.expander("Transcript", expanded=True):
            st.text(st.session_state.get("last_transcript", ""))
